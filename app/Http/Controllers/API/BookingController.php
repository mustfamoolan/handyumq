...
class BookingController extends Controller
{
    ...
    public function initiate(Request $request)
    {
        ...
        try {
            ...
            $phonepay=PaymentGateway::where('type','phonepe')->where('status',1)->first();


            if(!$phonepay){
                return response()->json(['error' => 'PhonePe payment gateway is not enabled.'], 400);
            }

            $phonepaymentSettings = json_decode($phonepay->value, true);



            if(empty($phonepaymentSettings['merchant_id']) || empty($phonepaymentSettings['salt_key'])) {
                return response()->json(['error' => 'PhonePe payment gateway is not configured properly.'], 400);
            }
            ...
            $merchantId = $phonepaymentSettings['merchant_id'];
            $apiPath = "/pg/v1/pay";
            ...
            $payload = [
                ...
                "amount" => $amount * 100,
                "redirectUrl" => url('/api/phonepe/callback?booking_id=' . $bookingId),
                "redirectMode" => "POST",
            ...
            $checksum = $this->generateChecksum($base64Payload, $apiPath,$phonepaymentSettings['salt_key'],$phonepaymentSettings['salt_index']);

            $response = Http::withHeaders([
                "Content-Type" => "application/json",
                "X-VERIFY" => $checksum,
                "X-MERCHANT-ID" => $merchantId
            ])->post($this->getBaseUrl($phonepay['is_test']) . $apiPath, [
    ...
    {
        // For POST requests, PhonePe sends data in the request body
        // For GET requests, data would be in query parameters
        $response = $request->all();

        // PhonePe sends data directly in the request body for POST callbacks
        // The data structure from the logs shows it's already decoded
        $decoded = $response;
        $status = 'FAILED';
        $merchantTransactionId = $decoded['data']['merchantTransactionId'] ?? null;
        $bookingId = $decoded['data']['booking_id'] ?? null;
        $phonePeTransactionId = $decoded['data']['transactionId'] ?? null;
        // Example structure: check transaction status
        if (isset($decoded['code']) && $decoded['code'] === 'PAYMENT_SUCCESS') {
            $status = 'SUCCESS';
            // update your record
            Payment::where('booking_id', $bookingId)->update([
                'payment_status' => 'paid',
                'txn_id' => $phonePeTransactionId,
            ]);
        } else {
            // Handle failure or pending
        }

        // Return a response to PhonePe
        return response()->json(['message' => 'Callback received', 'data' => $decoded]);
    }

    protected function generateChecksum($payload, $apiPath, $saltKey, $saltIndex=1)
    {

        $stringToHash = $payload . $apiPath . $saltKey;
        $hash = hash('sha256', $stringToHash);

        return $hash . "###" . $saltIndex;
    }

    protected function getBaseUrl($is_test)
    {
        return $is_test === 0
